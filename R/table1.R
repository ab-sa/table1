...
#'
#' @examples
table1 <- function(dataset, ContVarList = NULL, CatVarList = NULL, group = NULL, color_list = NULL) {

  if (is.null(ContVarList) & is.null(CatVarList)) stop("No variables selected!")
  ...
  if (! is.null(group)) {
    ...
    DataSummary_temp <- t(DataSummary[ , -1])
...
#'
#' @examples
DataSummary_Fnc <- function(dataset, ContVarList = NULL, CatVarList = NULL, group = NULL) {
